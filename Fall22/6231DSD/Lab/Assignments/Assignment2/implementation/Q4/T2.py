...
def map_tasks(reading_info: list, data: str = '../../Combined_Flights_2021.csv'):
    ...
    data3 = df[['DepTime', 'FlightDate']]
    data3[(data3['DepTime'].isna())]
    data3.reset_index().values.tolist()
    # print(data)
    return data3.values.tolist()
...
def reduce_task(mapping_output: list):
    ...
    result = pd.DataFrame(reduce_out.iloc[1:, :].groupby(1)).count()[0]
    print(result)
...
def compute_multiprocessing():
    ...
    print(distribute_rows(n_rows=row_subset, n_processes=processes))
